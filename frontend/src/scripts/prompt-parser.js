(function () {
  ...
  function parsePrompt(prompt) {
    const pageSize = readNumber(prompt, /每页\s*(\d+)\s*条/, 50);
    const sortField = findMatch(SORT_FIELD_MAP, prompt) || SORT_FIELD_MAP[0];
    const region = findMatch(SITE_REGION_MAP, prompt);

    return {
      platform: prompt.includes("TikTok Shop") ? "TikTok Shop" : "TikTok",
      region: region ? region.value : "未识别",
      keyword: readText(prompt, /商品关键词\s*([^，,。]+)/, "自行车"),
      category: readText(prompt, /商品分类\s*([^，,。]+)/, "未指定"),
      sortBy: sortField.label,
  ...
  function toEchoTikParams(query) {
    return {
      region: query.region,
      page_num: query.page,
  ...
  function findMatch(options, text) {
    return options.find((option) => text.includes(option.label));
  }

  window.ProductAgentPromptParser = {
    parsePrompt,
    toEchoTikParams
  };
})();
